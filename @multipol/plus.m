...
for i=numel(p1):-1:1
	if isnumeric(p1(i))
		...
		if ~isempty(p2(i).monomials) && ~isempty(ind)
			...
			p3(i) = multipol([p1(i) p2(i).coeffs],[zeros(size(p2(i).monomials,1),1) p2(i).monomials]);
		end
	...
	elseif isnumeric(p2(i))
		...
		if ~isempty(p1(i).monomials) && ~isempty(ind)
			...
			p3(i) = multipol([p2(i) p1(i).coeffs],[zeros(size(p1(i).monomials,1),1) p1(i).monomials]);
		end
